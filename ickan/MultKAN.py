...
class MultKAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, mult_arity = 2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun='silu', symbolic_enabled=True, affine_trainable=False, grid_eps=0.02, grid_range=[-1, 1], grid_range_0=[-1,1], sp_trainable=True, sb_trainable=True, seed=1, save_act=True, sparse_init=False, auto_save=True, first_init=True, ckpt_path='./model', state_id=0, round=0, device='cpu'):
        ...
        elif base_fun == 'softplus':
            base_fun = torch.nn.Softplus()
        elif base_fun == 'zero':
            base_fun = lambda x: x*0.
            
        self.grid_eps = grid_eps
        self.grid_range = grid_range
        self.grid_range_0 = grid_range_0

        for l in range(self.depth):
            if l == 0:
                sp_batch = KANLayer(in_dim=width_in[l], out_dim=width_out[l+1], num=grid, k=k, noise_scale=noise_scale, scale_base_mu=scale_base_mu, scale_base_sigma=scale_base_sigma, scale_sp=1., base_fun=base_fun, grid_eps=grid_eps, grid_range= self.grid_range_0, sp_trainable=sp_trainable, sb_trainable=sb_trainable, sparse_init=sparse_init, device=self.device)
            else:
        ...
        self.symbolic_fun = []
        for l in range(self.depth):
            sb_batch = Symbolic_KANLayer(in_dim=width_in[l], out_dim=width_out[l+1], device=self.device)
        ...
        self.adjust_init_grid()

    def adjust_init_grid(self):
        from .spline import extend_grid, coef2curve

        dummy_input = torch.zeros(self.width[0][0], 100, device=self.device)
        for i in range (len(self.grid_range_0)):
            dummy_input[i,:] = torch.linspace(self.grid_range_0[i][0],self.grid_range_0[i][1],100)
        
        dummy_input = dummy_input.transpose(0,1)
        self.get_act(dummy_input)
        for i in range(len(self.spline_preacts)):
            for j in range(self.spline_preacts[i].shape[2]):
                self.get_act(dummy_input)
                grid_tmp = torch.linspace(self.spline_preacts[i][:,:,j].min(), \
                                        self.spline_preacts[i][:,:,j].max(), \
                                        steps=self.grid + 1)[None,:]
                self.act_fun[i].grid[j:j+1,:] = torch.nn.Parameter(extend_grid(grid_tmp, k_extend=self.k)).requires_grad_(False)
        self.get_act(dummy_input)
    ...
    def fix_symbolic(self, l, i, j, fun_name, fit_params_bool=True, a_range=(0, 10), b_range=(-10, 10), verbose=True, random=False, log_history=True):
        '''
    ...
    def prune_node(self, threshold=1e-2, mode="auto", active_neurons_id=None, log_history=True):
        ...
        model2 = MultKAN(copy.deepcopy(self.width), grid=self.grid, k=self.k, base_fun=self.base_fun_name, mult_arity=self.mult_arity, ckpt_path=self.ckpt_path, auto_save=True, first_init=False, state_id=self.state_id, round=self.round, grid_range_0=self.grid_range_0).to(self.device)
    ...
    def auto_symbolic(self, a_range=(0, 10), b_range=(-10, 10), lib=None, verbose=1, weight_simple = 0.8, r2_threshold=0.0):
        '''
    ...
    def expand_depth(self):
        ...
        layer = Symbolic_KANLayer(dim_out, dim_out, device=self.device)
        layer.mask += 1.
    ...
    def expand_width(self, layer_id, n_added_nodes, sum_bool=True, mult_arity=2):
        ...
        def _expand(layer_id, n_added_nodes, sum_bool=True, mult_arity=2, added_dim='out'):
            ...
            if sum_bool:
                ...
                if added_dim == 'out':
                    new = Symbolic_KANLayer(in_dim, out_dim + n_added_nodes, device=self.device)
                ...
                if added_dim == 'in':
                    new = Symbolic_KANLayer(in_dim + n_added_nodes, out_dim, device=self.device)
                    old = self.symbolic_fun[l]
                ...
                if added_dim == 'out':
                    ...
                    new = Symbolic_KANLayer(in_dim, out_dim + n_added_subnodes, device=self.device)
                    old = self.symbolic_fun[l]
                    in_id = np.arange(in_dim)
                    out_id = np.arange(out_dim + n_added_nodes)
