...
SYMBOLIC_LIB = {'x': (lambda x: x, lambda x: x, 1, lambda x, y_th: ((), x)),
                 ...
                 'softplus': (lambda x: torch.log(1+torch.exp(x)), lambda x: sympy.log(1+sympy.exp(x)), 2, lambda x, yth:((), torch.log(1+torch.exp(x)))),
                 'softplus^2': (lambda x: torch.log(1+torch.exp(x))**2, lambda x: sympy.log(1+sympy.exp(x))**2, 2, lambda x, yth:((), torch.log(1+torch.exp(x))**2)),
                 'softplus^3': (lambda x: torch.log(1+torch.exp(x))**3, lambda x: sympy.log(1+sympy.exp(x))**3, 2, lambda x, yth:((), torch.log(1+torch.exp(x))**3)),
                 'softplus^4': (lambda x: torch.log(1+torch.exp(x))**4, lambda x: sympy.log(1+sympy.exp(x))**4, 2, lambda x, yth:((), torch.log(1+torch.exp(x))**4)),
...
def fit_params(x, y, fun, a_range=(0,10), b_range=(-10,10), grid_number=101, iteration=3, verbose=True, device='cpu'):
    '''
    ...
    reg = LinearRegression(positive=True).fit(post_fun[:,None].detach().cpu().numpy(), y.detach().cpu().numpy())
